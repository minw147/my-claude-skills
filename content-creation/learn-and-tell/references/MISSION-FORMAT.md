...
{Does the user already have a produced-content goal in mind — a video, a blog post, a specific angle? Or are they exploring freely for now with no target yet? Either is fine — this is a light note, not a commitment. Revisit/update whenever it changes.}

## Success looks like
- {A specific, observable thing the user will be able to explain or produce}
- {…}

## Constraints
- {Time, prior knowledge, things they explicitly don't want to chase right now}

## Out of scope
- {Adjacent topics deliberately excluded — protects the exploration from sprawling}
```
